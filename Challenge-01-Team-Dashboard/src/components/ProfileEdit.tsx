 ...
 *
 * The URL shapes are enforced by a database constraint, not just by this form,
 * so a hand-crafted request cannot put a javascript: link on a page teammates
 * are going to click.
 */
export default function ProfileEdit({
  id, department, github, linkedin, phone, onSaved,
}: {
  ...
  async function save() {
    ...
    if (error) {
      setErr(error.message.includes("shape")
        ? "A LINK IS THE WRONG SHAPE. GITHUB MUST START https://github.com/ AND LINKEDIN https://linkedin.com/"
  ...
  if (!open) {
    return (
      <button className="btn" onClick={() => setOpen(true)}>EDIT FILE</button>
    );
  }
  ...
  return (
    <div className="panel" style={{ padding: 18 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 12 }}>
        <label>
          <div className="lbl" style={{ marginBottom: 6 }}>GITHUB</div>
          <input className="fld" value={gh} onChange={(e) => setGh(e.target.value)} placeholder="https://github.com/you" />
        </label>
        <label>
          <div className="lbl" style={{ marginBottom: 6 }}>LINKEDIN</div>
          <input className="fld" value={li} onChange={(e) => setLi(e.target.value)} placeholder="https://linkedin.com/in/you" />
        </label>
        <label>
          <div className="lbl" style={{ marginBottom: 6 }}>WHATSAPP <span style={{ color: "var(--faint)" }}>· OPTIONAL</span></div>
